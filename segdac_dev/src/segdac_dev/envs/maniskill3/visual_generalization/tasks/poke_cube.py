...
from segdac_dev.envs.maniskill3.visual_generalization.tasks.scene_building import build_red_white_target
from segdac_dev.envs.maniskill3.visual_generalization.tasks.scene_building import build_two_color_peg
from mani_skill.utils.structs.pose import Pose


@register_env("PokeCubeTest-v1", max_episode_steps=50)
class PokeCubeTestEnv(PokeCubeEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        if mo_base_color is None:
            mo_base_color = np.array([12, 42, 160, 255]) / 255
        ...
        visual_file_path = "assets/peg/0_05_0_24/peg.glb"

        self.peg = build_two_color_peg(
            scene=self.scene,
            length=self.peg_half_length,
            width=self.peg_half_width,
            full_material=mo_material,
            part_1_material=None,
            part_2_material=None,
            name="peg",
            visual_file_path=visual_file_path,
            body_type="dynamic",
            initial_pose=sapien.Pose(p=[0, 0, self.peg_half_width]),
        )

        ro_cube_material_config = self.test_config["ro"]["cube"]["material"]
        ro_cube_base_color = ro_cube_material_config["base_color"]
        if ro_cube_base_color is None:
            ro_cube_base_color = np.array([255, 0, 0, 255]) / 255
        ro_cube_material = sapien.render.RenderMaterial(
            base_color=ro_cube_base_color,
        )
        if ro_cube_material_config["texture"] is not None:
            ro_cube_material.base_color_texture = sapien.render.RenderTexture2D(ro_cube_material_config["texture"])

        self.cube = build_cube(
            scene=self.scene,
            half_size=self.cube_half_size,
            material=ro_cube_material,
            name="cube",
            visual_file_path=ro_cube_material_config["visual_file_path"],
            body_type="dynamic",
            initial_pose=sapien.Pose(p=[1, 0, self.cube_half_size]),
        )

        ro_target_material_config = self.test_config["ro"]["target"]["material"]
        if ro_target_material_config["texture"] is not None:
            ro_target_material = sapien.render.RenderMaterial()
            ro_target_material.base_color_texture = sapien.render.RenderTexture2D(ro_target_material_config["texture"])
        else:
            ro_target_material = None

        ro_target_red_base_color = ro_target_material_config["red"]["base_color"]
        if ro_target_red_base_color is None:
            ro_target_red_base_color = np.array([194, 19, 22, 255]) / 255
        ro_target_white_base_color = ro_target_material_config["white"]["base_color"]
        if ro_target_white_base_color is None:
            ro_target_white_base_color = np.array([255, 255, 255, 255]) / 255
        ...
        self.goal_region = build_red_white_target(
            ...
            thickness=1e-5,
            name="goal_region",
            red_color=ro_target_red_base_color,
            white_color=ro_target_white_base_color,
            material=ro_target_material,
            visual_file_path=ro_target_material_config["visual_file_path"],
            add_collision=False,
            body_type="kinematic",
